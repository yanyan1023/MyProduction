var query=require('../../config/db_connect'),
    ...
    generateUUID = require("../common/Unique"),//生成唯一标识
    creatTime = require("../common/creatTime"), //创建时间
    moment = require('moment'); //格式化数据库时间
    router.post('/',(req,res,next)=>{
        ...
        query(sql,(err,rows,fields)=>{
            ...
            rows.forEach(function(i) {
                if(req.body.loginname===i.name){//用户名验证
                    user=true;
                    state=req.body.loginpwd===i.password;//密码验证
                    userI=i;
                }
            }, this);
            if(user){//如果用户名存在
